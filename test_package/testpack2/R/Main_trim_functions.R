...
rps10_barcode_function<- function(intermediate_path, cutadapt_data){
  ...
  tax_results_rps10_asv <- assign_taxonomyDada2(rps10_abund_asv, "rps10_reference_db.fa")
  ...
  dada2_readcounts_multi_sample(asv_abund_table)
}

#For single sample, rps10 barcode
rps10_barcode_function_single<- function(intermediate_path, cutadapt_data){
  infer_asv_command(intermediate_path, cutadapt_data)
  merged_reads<-merge_reads_command(intermediate_path)
  countOverlap(merged_reads)
  raw_seqtab<-makeSeqtab(merged_reads)
  ...
  dada2_readcounts_single_sample(asv_abund_table)
}
