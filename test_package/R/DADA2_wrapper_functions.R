...
remove_ns <- function(fastq_data, directory_path, maxN= 0, multithread = TRUE){
  ...
  fastq_data$prefiltered_path <- file.path(prefiltered_read_dir, base::basename(fastq_data$directory_data_path))
  ...
  if(! all(file.exists(fastq_data$prefiltered_path))){
    dada2::filterAndTrim(fwd = fastq_data[fastq_data$direction == "Forward", ][["directory_data_path"]],
...
plot_qc<-function(cutadapt_data, directory_path, n=500000){
  ...
  for (i in unique(cutadapt_data$sample_name))
  {
    ...
    ggplot2::ggsave(quality_plots, filename = name1, path = directory_path, width = 8, height = 8)
    #print(quality_plots)
  }
}
...
filter_and_trim <- function(directory_path, cutadapt_data,  maxEE = Inf, truncQ = 2, minLen = 20, maxLen = Inf, truncLen = 0, maxN = 0, minQ=0, rm.phix=TRUE, multithread=FALSE, matchIDs=FALSE, verbose=FALSE, qualityType="Auto", OMP=TRUE, n=1e+05,id.sep="\\s", rm.lowcomplex=0, orient.fwd=NULL, id.field=NULL){
  ...
  if(! all(file.exists(cutadapt_data$filtered_path))){
    ...
    filter_results_path<-file.path(directory_path, "filter_results.RDS")
    saveRDS(filter_results, file = filter_results_path)
    filter_results_path2<-file.path(directory_path, "filter_results.RData")
...
plot_post_trim_qc<-function(cutadapt_data, directory_path, n=500000){
  ...
  for (i in unique(cutadapt_data$sample_name))
  {
    ...
    ggplot2::ggsave(quality_plots2, filename = name, path = directory_path, width = 8, height = 8)
  ...
  formatted_ref_dir <-
    if (file.exists(merged_read_data_path)) {
      ...
      merged_reads <- dada2::mergePairs(dadaF = dada_forward,
                                        derepF = file.path(directory_path, 'filtered_sequences', names(dada_forward)),
                                        dadaR = dada_reverse,
                                        derepR = file.path(directory_path, 'filtered_sequences', names(dada_reverse)),
...
countOverlap <- function(merged_reads){
  ...
  ggsave(merge_plot, filename = 'read_merging.jpg', path = directory_path, width = 8, height = 8)
  merge_plot
}
...
make_abund_matrix<-function(raw_seqtab, method="consensus", verbose=FALSE, min_asv_length=50){
  ...
  asvabund_matrix_path <- file.path(directory_path, "asvabund_matrixDADA2.Rdata")
...
make_seqhist <- function(asv_abund_matrix){
  ...
  hist_plot<-ggplot2::qplot(ab_matrix, data=data1, geom="histogram", xlab = 'Length of sequence (bp)', ylab='Counts', main='Read length counts of all sequences within the ASV matrix')
  ggsave(hist_plot, filename = "asv_hist_plot.pdf", path = directory_path, width = 8, height = 8)
}
...
#'
#' @examples
separate_abund_matrix <- function(abund_asv_its, abund_asv_rps10, directory_path, asv_abund_matrix){
