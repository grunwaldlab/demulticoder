...
prepare_fastq <- function(raw_path,intermediate_path, maxN= 0, multithread = FALSE){
  ...
  fastq_data <- remove_ns(fastq_data, intermediate_path, maxN, multithread)
...
prepare <- function(directory_path, primer_path, metadata_path, fastq_path,intermediate_path){
  intermediate_path <- create_intermediate(directory_path)
  primer_data <- prepare_primers(primer_path)
  metadata <- prepare_metadata(metadata_path, primer_data)
  fastq_data <- prepare_fastq(raw_path, intermediate_path)
  ...
  cutadapt_data <- cutadapt_tibble(fastq_data, metadata, intermediate_path)
  returnList <- list(cutadapt_data, primer_data, fastq_data, metadata)
  return(returnList)
}
...
cut_trim <- function(returnList,intermediate_path,cutadapt_path){
  cutadapt_run(cutadapt_path, returnList[1])
  quality_plots<-plot_qc(returnList[1], intermediate_path)
  filter_results <-filter_and_trim(intermediate_path, returnList[1])
  post_primer_hit_data <- post_primer_hit_data(returnList[2], returnList[1],
                                               intermediate_path)
  post_primer_plot <- primer_hit_plot(post_primer_hit_data, returnList[3],
                                      intermediate_path, "post_primer_plot.pdf")
  quality_plots2 <- post_trim_qc(returnList[1], intermediate_path)
  return(returnList)
}