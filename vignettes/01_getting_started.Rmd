...
There will be an error because R1 and R2 are not directly preceding the '.fastq.gz' suffix.
...
To simplify how functions are called, user will provide parameters within this input file. We recommend using the template linked [here](https://github.com/grunwaldlab/demulticoder/blob/main/inst/extdata/primerinfo_params.csv)
...
If a user doesn't add any info to the subsequent columns, a series of default parameters will be used.
...
TODO-Below is a table of the parameters, what functions they are associated with,  the default parameters, and whether they are Cutadapt, DADA2, or package specific parameters.  
...
```{r prepare_reads function, warning=FALSE, message=FALSE, fig.height=8, fig.width=10}
...
```{r remove primers with cut_trim function, message=FALSE, fig.height=6, fig.width=6}
...
```{r ASV inference, message=FALSE, fig.height=8, fig.width=6}
...
```{r Assign taxonomy, message=FALSE, fig.height=6, fig.width=6}
assign_tax(
  analysis_setup,
  asv_abund_matrix,
  retrieve_files=TRUE,
...
```{r Reformat final matrix, message=FALSE, fig.height=8, fig.width=6}
objs<-convert_asv_matrix_to_objs(analysis_setup, save_outputs=TRUE)
```

### Now we demonstrate how to make a heattree using taxmap object. First we make a heat tree for our ITS-barcoded samples 
```{r Heatree demonstration its, message=FALSE, fig.height=6, fig.width=6}
...
```{r Heatree demonstration rps10, message=FALSE, fig.height=6, fig.width=6}
...
```{r Phyloseq demonstration ITS, message=FALSE, fig.height=6, fig.width=6}
...
```{r Phyloseq demonstration rps10, message=FALSE, fig.height=6, fig.width=6}
