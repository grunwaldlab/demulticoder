...
outputs<-prepare_reads(
  ...
  output_directory = "~/demulticoder_benchmarking/demulticoder_combined_analysis_dads/output", 
...
Here is an example with the 16S dataset. For whatever reason, there is an error with igraph, so figure isn't rendering, even though intermediate outputs look reasonable. Will look at this again. Other two trees rendered properly. 
...
objs$taxmap_its<- filter_taxa(objs$taxmap_its, taxon_names == "Fungi", subtaxa = TRUE)
objs$taxmap_its <- filter_taxa(objs$taxmap_its, taxon_names != "")
