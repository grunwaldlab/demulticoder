...
if (!overwrite_existing) {
  knitr::include_graphics("./posttrim_primer_plot_pooled.pdf")
}
```
...
if (!overwrite_existing) {
  knitr::include_graphics("./read_merging_info_rps10.png")
}
```
...
if (!overwrite_existing) {
  knitr::include_graphics("./read_merging_info_its.png")
}
```
...
```{r visualize_outputs_asv_inf3, fig.height=10, fig.width=10, echo=FALSE}
if (!overwrite_existing) {
  knitr::include_graphics("./asv_seqlength_plot_rps10.png")
}
```

Then we see the ITS plots  

```{r visualize_outputs_asv_inf4, fig.height=10, fig.width=10, echo=FALSE}
if (!overwrite_existing) {
  knitr::include_graphics("./asv_seqlength_plot_its.png")
}
```
...
if (!overwrite_existing) {
  track_path <- "./track_reads_rps10.csv"
   read_track_file<-read.csv(track_path)
   head(read_track_file)
}
```

Then we track *ITS1* reads  

```{r view_read_counts2, fig.height=10, fig.width=10, echo=FALSE}
if (!overwrite_existing) {
  cat("Tables showing read counts throughout the demulticoder DADA2 workflow\n")
 track_path <- "./track_reads_its.csv"
   read_track_file<-read.csv(track_path)
   head(read_track_file)
}
```
