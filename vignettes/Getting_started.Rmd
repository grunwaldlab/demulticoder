...
### 3. Format of primer and parameters file (*primerinfo_parms.csv*)
...
* **metacoder** (Available through CRAN)  
  * If you are using the latest version of R (4.4.2) and R studio (2024.12.0+467), you may temporarily need to install metacoder through Github, until CRAN approves the latest version.
...
* You can then open your matrices using Excl We recommend checking the column with the ASV sequences and also the column showing the taxonomic assignments of each ASV and their associated bootstrap supports. 

We can also load the matrices into our R environment and view then from R Studio. We do that here.  
```{r Show_rps10_matrix, message=TRUE, warning=FALSE}
...
```{r Show_its_matrix, message=TRUE, warning=FALSE}
...
First we make a heat tree for our **ITS1**-barcoded samples. We retain only taxa that have bootstrap supports > 60 and filter out any with the classification 'Unsupported' or that didn't have an assignment and are marked as 'NA'.   

```{r Heatree_demonstration_its, fig.height=8, fig.width=8, echo=TRUE, results='hide', message=FALSE, warning=FALSE}
objs$taxmap_its %>%
...
```{r Heatree_demonstration_rps10, fig.height=8, fig.width=8, echo=TRUE, results='hide', message=FALSE, warning=FALSE}
objs$taxmap_rps10 %>%
  filter_taxa(! grepl(x = taxon_names, "NA", ignore.case = TRUE)) %>%
  metacoder::heat_tree(node_label = taxon_names,
                       node_size = n_obs,
...
```{r Phyloseq_demonstration_ITS, fig.height=8, fig.width=8, echo=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r Phyloseq_demonstration_rps10, fig.height=8, fig.width=8, echo=TRUE, results='hide', message=FALSE, warning=FALSE}
