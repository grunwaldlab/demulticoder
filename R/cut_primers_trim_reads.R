...
run_cutadapt <- function(cutadapt_path,
  ...
  R1_flags = unique(paste(
    "-g",
    cutadapt_data_barcode$forward,
    "-a",
    cutadapt_data_barcode$r_rc
  ))
  R2_flags = unique(paste(
    "-G",
    cutadapt_data_barcode$reverse,
    "-A",
  ...
  if (!all(file.exists(c(cutadapt_data_barcode$trimmed_path))) &&
      !barcode_params$already_trimmed) {
    cutadapt_output <-
      furrr::future_map(command_args, ~ system2(cutadapt, args = .x))
  }
  
  else if (!all(file.exists(c(cutadapt_data_barcode$trimmed_path))) &&
           barcode_params$already_trimmed) {
    cutadapt_output <-
      furrr::future_map(command_args, ~ system2(cutadapt, args = .x))
  ...
  if (!all(file.exists(cutadapt_data_barcode$filtered_path))) {
    ...
    filter_results <-
      dada2::filterAndTrim(
        ...
        maxEE = c(
          barcode_params$maxEE_forward,
          barcode_params$maxEE_reverse
        ),
        truncLen = c(
          barcode_params$truncLen_forward,
          barcode_params$truncLen_reverse
        ),
    ...
    filter_results_path <-
      file.path(temp_directory_path,
                paste0("Filter_results_", barcode, ".RData"))
...
get_post_trim_hits <-
  function(primer_data,
    ...
    post_primer_hit_data_csv_path <-
      file.path(output_directory_path, "primer_hit_data_posttrim.csv")
    post_primer_hit_counts <-
      furrr::future_map(cutadapt_data$filtered_path,
                        function(a_path)
    ...
    post_trim_hit_data <-
      dplyr::bind_cols(post_trim_hit_data,
                       dplyr::as_tibble(post_primer_hit_counts))
    ...
    make_posttrim_primer_plot <- function(post_trim_hits,
      ...
      post_trim_hits$primer_type <-
        paste(post_trim_hits$primer_name, post_trim_hits$orientation)
      ...
      ggplot2::ggsave(
        plot,
        filename = file.path(output_directory_path, "posttrim_primer_plot.pdf"),
        width = 8,
        height = 8
      )
...
cut_trim <- function(analysis_setup,
  ...
  patterns_to_check <- c("primer_hit_data_posttrim.csv",
  ...
  if (any(files_exist)) {
    if (!overwrite_existing) {
      ...
      patterns_to_remove_temp <- c("Filter_results_*")
      ...
      temp_untrimmed <-
        file.path(temp_directory_path, "untrimmed_sequenced")
      temp_trimmed <-
      ...
      if (dir.exists(temp_untrimmed)) {
        unlink(list.files(temp_untrimmed, full.names = TRUE),
               recursive = TRUE)
      }
      if (dir.exists(temp_trimmed)) {
        unlink(list.files(temp_trimmed, full.names = TRUE),
               recursive = TRUE)
      }
      if (dir.exists(temp_filtered)) {
        unlink(list.files(temp_filtered, full.names = TRUE),
               recursive = TRUE)
      }
      
      subdirectory_names <-
        c("filtered_sequences",
          "trimmed_sequences",
          "untrimmed_sequences")
      ...
      for (seqdir_name in subdirectory_names) {
        ...
        if (dir.exists(seqdir_path)) {
          unlink(list.files(seqdir_path, full.names = TRUE),
                 recursive = TRUE)
        }
      }
    }
  ...
  for (barcode in unique_primers) {
    ...
    if (nrow(barcode_params) > 0) {
      ...
      barcode_params <-
        utils::modifyList(default_params, barcode_params)
      
      cutadapt_data_barcode <-
        subset(data_tables$cutadapt_data, primer_name == barcode)
      if (nrow(cutadapt_data_barcode) > 0 &&
          !all(file.exists(c(
            cutadapt_data_barcode$trimmed_path
          )))) {
        run_cutadapt(
          cutadapt_path,
          cutadapt_data_barcode,
        ...
        post_primer_hit_data <-
          get_post_trim_hits(data_tables$primer_data,
                             cutadapt_data_barcode,
                             output_directory_path)
        plot_post_trim_qc(cutadapt_data_barcode,
                          output_directory_path,
                          barcode_params = barcode_params)
      }
    }
