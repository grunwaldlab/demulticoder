...
#'
#' @examples
format_database_unite <-function(directory_path, database_its){
...
#TODO Make 16S database? function to combine rps10 and unite database
#format_database_all
...
#'
#' @examples
format_database_silva <-function(directory_path, database_16s){
  database_path <- file.path(directory_path, "its_reference_db.fa")
  its_db <- read_fasta(file.path(directory_path, database_its))
  its_data <- str_match(names(its_db), pattern = "(.+)\\|(.+)\\|(.+)\\|(.+)\\|(.+)$")
  colnames(its_data) <- c("header", "name", "ncbi_acc", "unite_db", "db", "taxonomy")
