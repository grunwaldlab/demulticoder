...
prep_abund_matrix <-
  function(cutadapt_data,
           ...
           data_tables,
           metabarcode) {
    data_tables$cutadapt_data$file_id_primer <-
      paste0(
        data_tables$cutadapt_data$sample_name,
        "_",
  ...
  function(asv_abund_matrix,
    ...
    refFasta_raw <-
      file.path(temp_directory_path,
                paste0(metabarcode, "_reference_db.fa"))
    refFasta <-
      file.path(temp_directory_path,
                paste0(metabarcode, "_reference_db_unique.fa"))
    ...
    if (metabarcode %in% c("rps10")) {
      ...
      tax_results <- dada2::assignTaxonomy(
        asv_abund_matrix,
        refFasta,
        taxLevels = c(
          "Kingdom",
    ...
    tax_matrix_path <-
      file.path(temp_directory_path,
                paste0("TaxMatrix_", metabarcode, ".RData"))
...
assign_tax <-
  function(analysis_setup,
    ...
    files_to_check <-
      c("*_reference_db.fa", "TaxMatrix_*", "Final_tax_matrix_*")
    existing_files <-
    ...
    if (!overwrite_existing && length(existing_files) > 0) {
      ...
      patterns_to_remove_temp <- c("*_reference_db.fa",
                                   "TaxMatrix_*",
